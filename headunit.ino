...
void loop() {
  ...
  draw_title(title.c_str(), ST7735_WHITE);
  draw_artist(artist.c_str(), ST7735_WHITE);
  draw_album(album.c_str(), ST7735_WHITE);
  Serial.println(data);
...
String get_metadata(int* i){
  ...
  Serial2.print("AD\r");
  while(!Serial2.available()); //Wait until there is stuff on the line
  while( complete < 7){
...
String get_artist(String metadata){
    int i = metadata.indexOf("Artist=") + 7;
    if (i != 6){ // -1 + offset
      metadata.remove(0,i); //Removes everything up to the artist
...
String get_title(String metadata){
    int i = metadata.indexOf("Title=") + 6;
    if (i != 5){// -1 + offset
      metadata.remove(0,i); //Removes everything up to the artist
      i = metadata.indexOf('\n'); //Gets end of line after artist
      metadata.remove(i);
...
String get_album(String metadata){
    int i = metadata.indexOf("Album=") + 6;
...
void draw_title(const char *text, uint16_t color) {
  tft.setCursor(0, (TITLE_OFFSET * CHAR_SIZE) + V_BUFF);
  tft.setTextColor(color);
  tft.setTextWrap(true);
...
void draw_artist(const char *text, uint16_t color) {
  tft.setCursor(0,(ARTIST_OFFSET * CHAR_SIZE) + V_BUFF);
  tft.setTextColor(color);
  tft.setTextWrap(true);
  tft.setTextSize(SIZE_MULT);
  tft.print("Artist: ");
...
void draw_album(const char *text, uint16_t color) {
  tft.setCursor(0,(ALBUM_OFFSET * CHAR_SIZE) + V_BUFF);
  tft.setTextColor(color);
  tft.setTextWrap(true);
  tft.setTextSize(SIZE_MULT);
  tft.print("Album: ");
